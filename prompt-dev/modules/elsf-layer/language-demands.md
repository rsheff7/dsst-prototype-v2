...
ELSF does NOT replace the MLR layer; it deepens the language reasoning that downstream guidance (friction points, proficiency adaptations, sentence frames) draws on. The two layers work together: ELSF diagnoses an activity's Language Mode; MLRs name the response vocabulary the teacher uses for language-rich moments.

For each activity in the lesson, you must produce an elsf_inference entry with two structured blocks.

LANGUAGE_DEMANDS — the Language Mode: how students engage with language in this activity. Language Mode characterises the activity; it does not select the routine and does not determine the Key Language Use:
  ...
  - interactive: what back-and-forth exchange happens with peers
...
The ELSF reasoning must inform what you produce downstream. Specifically:
  - by_proficiency adaptations should reflect the bridge each level needs (Emerging students need more receptive scaffolding; Expanding students need finer functional language)
